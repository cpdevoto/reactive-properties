...
The framework also supports the notion of nested value modifiers, which allow you to decorate a property's value instead of changing it every time you want to adjust the formula. In the example cited above, a character might acquire a set of Gauntlets of Ogre Power that raise his strength property to a 19 if it is currently below 19, but have no effect if the character's strength is already 19 or higher. This can easily be implemented as a modifier function applied to the strength property.  The modifier function takes the character's strength score as an input and returns the adjusted strength score. Multiple modifiers can be applied to the same property, and you are provided with several ordering rules for precise control over the order in which the modifiers are executed (i.e. applyFirst(), applyLast()). Modifiers are extremely useful in that they allow you to alter a value computation at runtime at runtime as opposed to compile time.  This can be done by adding and removing predefined modifier functions at runtime in response to user actions.
